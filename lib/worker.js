...
function pickAndLockJob() {
  // only pick jobs with state='pending' and available_at <= now
...
async function processJob(job) {
  ...
  try {
    ...
    // in case of unexpected error, ensure job returned to pending for retry
...
async function mainLoop(pollIntervalMs = 1000) {
  ...
  // wait until current job finished
  while (currentJobId) {
