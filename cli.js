...
program
  ...
  .action((json) => {
    ...
    const maxRetries = job.max_retries != null ? job.max_retries : getInt('default_max_retries', 3);
    db.prepare(`
...
program
  ...
  .action((opts) => {
    let rows;
    if (opts.state) {
      rows = db.prepare('SELECT * FROM jobs WHERE state = ? ORDER BY created_at DESC').all(opts.state);
    } else {
      rows = db.prepare('SELECT * FROM jobs ORDER BY created_at DESC LIMIT 200').all();
    }
...
program
  ...
  .action((opts) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      console.log(`Started worker pid=${child.pid} (interval=${interval}ms)`);
    }
  });

program
  .command('worker stop')
  .description('Stop all worker processes gracefully (by sending SIGTERM)')
  .action(() => {
    const files = fs.readdirSync(WORKERS_DIR).filter(f => f.endsWith('.pid'));
    files.forEach(f => {
      const pid = parseInt(fs.readFileSync(path.join(WORKERS_DIR, f), 'utf8'), 10);
      try {
      ...
      try { fs.unlinkSync(path.join(WORKERS_DIR, f)); } catch (e) {}
    });
  });
...
program
  ...
  .action((jobId) => {
    ...
    db.prepare('UPDATE jobs SET state = "pending", attempts = 0, updated_at = datetime("now"), available_at = strftime("%s","now") WHERE id = ?').run(jobId);
